...
//Tank.prototype.findNearestTank = function() {
//    var min_dist = 0;
//    var nearest_id = 0;
//    var me_name = this.name;
//    for (var i = 0; i < tanks.length;i ++ ) {
//        if (tanks[i].name === me_name) { continue; }
//        var dis = Math.abs(tanks[i].x - this.x) + Math.abs(tanks[i].y - this.y);
//        if (dis > min_dist) {
//            min_dist = dis;
//            nearest_id = i;
//        }
//    }
//    return tanks[nearest_id];
//};

Tank.prototype.getNextTarget = function () {
//    if (this.target === -1) {
//        var meIndex = tanks.indexOf(this);
//        console.log(meIndex, this.target, tanks.length);
//        do {
//            this.target = Math.floor(tanks.length * Math.random());
//        } while (this.target === meIndex && tanks.length > 1);
//    }
//    if (this.target === -1) {
//        return tanks[0];
//    } else {
//        return tanks[this.target];
//    }
    function findNextTank(me) {
        if (me.target === null) {
            do {
                var randomTank = tanks[Math.floor(tanks.length * Math.random())];
            } while (randomTank === me && tanks.length > 1);
//            console.log(tanks.length, me.name, randomTank.name);
            if (randomTank === me) {
                me.target = null;
                return null;
            } else {
                me.target = randomTank.name;
                return randomTank;
            }
        }
        for (var i = 0; i < tanks.length; i++) {
           if (tanks[i].name === me.target) {
                return tanks[i];
            }
        }
        return null;
    ...
    var tank = findNextTank(this);
    if (tank === null && this.target !== null) {
        this.target = null;
        return findNextTank(this);
    } else {
        return tank;
    }
};
...
Tank.prototype.think = function(amap) {
  ...
  y = pix2block(this.y);
  if (home_is_grid()) {
//      var nearestTank = this.findNearestTank();
      var nextTank = this.getNextTarget();
      if (nextTank) {
          p = astar(x, y, pix2block(nextTank.x), pix2block(nextTank.y), amap, aux_map);
      } else {
          return ;
      }
  } else {
