...
Bullet.prototype.hitTanks = function()
{
    for(var i = 0;i < tanks.length;i ++ )
    {
        ...
        {
//            console.log(this.name, tanks[i].name, tanks[i].type);
            if (this.name == tanks[i].name) {
                continue;
            }
...
//            if(this.type == 0)                                        //�ӵ����Լ������
            ...
            if(!isBomb && tanks[i].type == 0 && !tanks[i].isGod)
            {
//                tanks[i].live --;
            ...
            // if(tankNum >= 20 && tanks.length == playerNum) nextStage(); // only 1 stage
            return true;
        }
    }
    return false;
        ...
        if(bullets[i].type == 0)
        {
            for(var j = 0;j < bullets.length;j++)
            {
                    ...
                    else if(bullets[i].hitTestObject(bullets[j]))// && bullets[i].type == 0)
                    {
                        ...
                        else if(bullets[i].name == 2) {player2.time = player2.shotSpeed - 14;}
                        else {continue;}
